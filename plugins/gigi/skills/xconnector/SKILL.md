...
Selectează etichetele **nedescărcate** (`downloaded=false` = coada de print), le **descarcă** (PDF), le pune într-un **batch PDF merged** în ordinea grupată, scrie un **log CSV cu `downloaded_at`** (audit „când s-a printat"), apoi **deschide dialogul de print** (macOS Preview + Cmd⌘P). Rulează **LOCAL** (mașina cu imprimanta — are uv + acces la secrete).
