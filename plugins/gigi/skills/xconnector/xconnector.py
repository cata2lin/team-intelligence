...
def nomenclator_correct(xc, o, shop_domain, mcur, apply=False):
    ...
    if stt == "corrected" and r.get("address"):
        ...
        note = r.get("note", "")
        # aceeași poartă și pe corecție: nu scriem un oraș pe care DPD îl refuză
        _dcity, _dwhy = _dpd_city_gate(mcur, applied.get("city"), applied.get("province"))
        if _dwhy and _dwhy.startswith("dpd-fix:"):
            applied["city"] = _dcity; note = (note + " +" + _dwhy).strip()
        elif _dwhy == "dpd-necunoscut":
            return "cs", None, "DPD nu cunoaște localitatea „%s\" după corecție" % applied.get("city")
        ...
        if o.get("addressStatus") in ("WRONG", "UNKNOWN"):
            ...
            if _snap:
                ...
                if not _same:
                    ...
                    return ("corrected" if ok else "error:write"), applied, detail
        # POARTA DPD: nomenclatorul poștal poate zice „valid" pe o localitate pe care
        # curierul o respinge (cartier listat ca localitate). Verificăm ÎNAINTE de AWB.
        _dcity, _dwhy = _dpd_city_gate(mcur, base.get("city"), base.get("province"))
        if _dwhy and _dwhy.startswith("dpd-fix:"):
            applied = dict(base); applied["country"] = "Romania"; applied["city"] = _dcity
            detail = "%s, %s %s (%s) [%s]" % (applied.get("address1"), applied.get("city"),
                                              applied.get("zip"), applied.get("province"), _dwhy)
            if not apply:
                return "would-correct", applied, detail
            ok = _nomen_write(xc, oid, d, ad, applied, shop_domain)
            awb_event(kind="dpd-city-fix", store=shop_domain, order=d.get("orderName"),
                      result="ok" if ok else "fail", detail=("%s→%s" % (base.get("city"), _dcity))[:60])
            return ("corrected" if ok else "error:write"), applied, detail
        if _dwhy == "dpd-necunoscut":
            return "cs", None, "DPD nu cunoaște localitatea „%s\" (nomenclatorul poștal o acceptă)" % base.get("city")
...
# Redenumit din `_ST_TYPE_RE`: numele se ciocnea cu globalul de la linia 791, folosit
# de `_tok_words` ca STRING. Fiind definit la import ca Pattern, garda `is None` de acolo
# nu se mai declansa si concatenarea crapa cronul de corectie adrese (385 de esecuri in log).
...
def awbprint_phone_fp(order_number):
    """(store_uid, phone9, total, currency, nsku, created) din AWBprint, sau None. phone9 = ultimele 9
    cifre din shipping_address.phone. Baza gardei de duplicat pe TELEFON (email/nume/GID pot diferi:
...
def awbprint_phone_sibling(fp, order_number, window_hours=48):
    ...
    AWB (tracking_number not null, necancelat). Intoarce numele fratelui sau None. Prinde dublura pe
    telefon scapata de tag/email (cross-run: AWBprint deja reflecta fratele expediat)."""
...
def _dpd_localitate_de_baza(mcur, city, judet):
    """Localitatea REALĂ al cărei nume e începutul celei date, în același județ.
    „Râşnov Romacril" → „Râşnov" (Romacril e CARTIER, dar nomenclatorul poștal îl listează
    ca localitate separată). Data-driven din `romania_addresses`, fără listă scrisă de mână.
    Potrivirea se face în Python — baza `metrics` NU are extensia `unaccent`."""
    cf = _fold(city or "")
    if not cf or not mcur:
        return None
    try:
        mcur.execute("SELECT DISTINCT localitate, judet FROM romania_addresses WHERE judet IS NOT NULL")
        jf = _fold(judet or "")
        best = None
        for loc, jud in mcur.fetchall():
            if jf and _fold(jud or "") != jf:
                continue
            lf = _fold(loc or "")
            if lf and lf != cf and cf.startswith(lf + " "):
                if best is None or len(lf) > len(_fold(best)):
                    best = loc
        return best
    except Exception:
        return None
...
def _dpd_city_gate(mcur, city, judet):
    """Poartă înainte de scriere: acceptă DPD localitatea asta?

    De ce există: nomenclatorul POȘTAL și lista DPD nu coincid. `Râşnov Romacril` e o
    localitate perfect validă poștal, dar DPD n-o cunoaște și respinge coletul — iar noi
    aflam abia la emiterea AWB-ului, fiindcă `nomenclator_correct` nu întreba curierul.

    Întoarce (oras_de_folosit, motiv) unde motiv e:
      None            → DPD acceptă, nu schimbăm nimic
      'dpd-fix:<X>'   → DPD refuză, dar acceptă localitatea de bază <X> → o folosim
      'dpd-necunoscut'→ DPD refuză și n-avem alternativă → la CS, NU scriem o adresă moartă

    FAIL-OPEN pe eroare de API: dacă DPD nu răspunde, întoarcem (city, None) — o pană de
    rețea nu are voie să rescrie adrese sau să blocheze comenzi.
    """
    if not (city or "").strip():
        return city, None
    try:
        site, why = dpd_site_by_city(DPD_COUNTRY_ID.get("RO", 642),
                                     city, judet or "", None)
    except Exception:
        return city, None          # API mort → nu ne atingem de comandă
    if site:
        return city, None
    if why in ("eroare-api", "fara-oras"):
        return city, None          # idem: nu e un refuz, e o pană
    baza = _dpd_localitate_de_baza(mcur, city, judet)
    if baza:
        try:
            s2, _ = dpd_site_by_city(DPD_COUNTRY_ID.get("RO", 642),
                                     baza, judet or "", None)
        except Exception:
            return city, None
        if s2:
            return baza, "dpd-fix:%s" % baza
    return city, "dpd-necunoscut"
...
def cmd_fulfill(a):
    ...
    made_ident = set()
    made_phone = set()  # garda pe TELEFON: (magazin, tel9, suma, moneda, nr_sku) carora le-am facut AWB in tura asta
                ...
                if not _exempt:
                    ...
                    if _pfp:
                        ...
                        if _psib or _pkey in made_phone:
                            # STRICT IDENTIC (aceleași SKU-uri, via awbprint_compare) + NEplătit card → ANULEZ (ca regula 4b, restock ON).
                            # Card PAID → NU anulez (chargeback) → HOLD. Same-run (fără nume frate) → HOLD (conservator).
                            _ident_dup = False
