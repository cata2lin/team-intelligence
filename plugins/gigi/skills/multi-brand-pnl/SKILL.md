...
> ✅ **SURSA CANONICĂ de profit net pe brand** (delivered only, ex-TVA, − COGS − transport − marketing, din
> `cache.brand_pnl_monthly`). Când alt skill dă alt „profit" (spend-pacing MER, cpa-report Sheet, grandia-pnl,
> ha-grandia-pnl), ĂSTA e reperul. ⚠️ **`--estimat` (daily_perf, gross-cu-TVA, toate comenzile) SUPRAESTIMEAZĂ
> profitul — DEPRECAT**, folosește-l doar pt viteză daily, nu pt decizii. Vezi `shared/skills-audit.md` §5.1.
...
> 2. **MARKETINGUL** = `cache.daily_ad_spend_ron` (warehouse) e **CORECT** (aplică Mapping-ul cont/token din „CPA și financiar"). ⚠️ NU re-deriva din spend BRUT pe cont (conturi PARTAJATE: „Esteban 3"=Ofertele!). ✅ **Grandia Meta REPARAT 29-iun** (warehouse corect 51k; era ×curs spurios pe FB în tabul „Grandia"). Excepție rămasă: **Belasil TikTok token-gol** (split, suma neschimbată). Verificare = `gigi:meta-ads`/`tiktok-ads` (aplică maparea), NU spend brut.
