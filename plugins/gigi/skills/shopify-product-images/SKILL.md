...
2. **`productDeleteMedia` DOAR DETAȘEAZĂ** de la produsul dat — NU șterge global. Co-partajorii rămân intacți (verificat). Deci replace pe prima poză (upload nou + reorder poz.0 + delete vechea) e sigur și pe media partajat.
   ⚠️ **REDENUMIREA SPARGE REFERINȚELE DIN TEMĂ.** Temele referă imagini prin `shopify://shop_images/<filename>` — INCLUSIV poze de PRODUS folosite decorativ în secțiuni (ex. Lab Noir „Calitate franțuzească" referea `35_scandal_homme_5.jpg`). Când redenumești media (`fileUpdate filename`), aceste referințe rămân la numele VECHI → **placeholder** pe home/produs. ÎNAINTE de a redenumi în masă: `grep` prin `templates/*.json` + `config/settings_data.json` după `shop_images\\?/` și notează ce nume sunt folosite. DUPĂ: verifică ce s-a spart (`files(query:"filename:X")`) și repară fie **reurcând originalul sub numele vechi** (fileCreate din backup — cel mai curat, fixează home+produs deodată, fără editare de temă), fie re-pointând setarea din JSON. Backup-ul de originale e obligatoriu tocmai pentru asta.
