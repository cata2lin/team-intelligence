...
def refusal_transport_multiplier(status_category: str, is_intl: bool = False) -> float:
    """Multiplicator pe cost_per_parcel: orice colet PLECAT (Livrata / În curs / Refuzat) → ×1; altele → 0.
    NU se dublează pe internațional: `cost_per_parcel` pt intl e DEJA mai mare (CZ 22.5, PL 25 vs RO 13) și
    reflectă deja shipping-ul intl; iar pe RO nu există cost de retur. (is_intl păstrat pt compat — neutilizat.)"""
    if status_category in ("Livrata", "In curs de livrare", "Refuzata"):
        return 1.0
    return 0.0
