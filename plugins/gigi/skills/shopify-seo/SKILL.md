...
- **Descrieri LSI pe colecții = metafield `custom.seo_lsi` de tip `rich_text_field` (NU HTML!).** Format = JSON Shopify: `{"type":"root","children":[{"type":"heading","level":2,"children":[{"type":"text","value":"…"}]},{"type":"paragraph",…},{"type":"list","listType":"unordered","children":[{"type":"list-item",…}]}]}` (text node acceptă `"bold":true`). Se randează prin **binding dinamic** în template: setting-ul secțiunii = `{{ collection.metafields.custom.seo_lsi | metafield_tag }}` (Liquid convertește rich-text→HTML). Stil Limitless: H2 + intro + ghid „Cum alegi" + listă beneficii + FAQ. Plasament: ideal SUB produse (Google citește below-the-fold; userii văd produsele primii). **⚠️ DAR cu INFINITE SCROLL „sub produse" = de neatins** — pagina încarcă toate produsele, URL-ul devine `?page=N`, textul ajunge la ~19.000px sub tot. Soluție: pune-l la finalul colecției DAR **deasupra secțiunii de review/blog** (nu chiar ultimul), sau sus colapsat; și **relaxează hide-on-filter să IGNORE `?page=`** (ascunde doar pe `filter.`/`sort_by`), altfel paginarea din infinite scroll îl ascunde.
...
- **Ierarhia de heading-uri pe pagina de categorie (validată Limitless):** exact **1× H1 = numele curat al categoriei** („Mobilier"); **H2 deasupra produselor** (înainte de bara de sortare) = keyword-rich cu intenție de listare („Obiecte de mobilier - Vezi lista completa de produse"); **H3 sub produse** = un CTA („Descoperă colecția…"). **Numele produselor din carduri + titlul secțiunii de blog NU trebuie să fie heading-uri** (`<h3>/<h4>`) → convertește-le în `<div>/<span>` (păstrezi vizualul prin CSS) ca să nu dilueze densitatea de cuvinte cheie. Userul poate percepe H1+H2 ca „două titluri" — e intenționat (H1 brandabil + H2 keyword), nu bug.
- **AEO — conținutul SEO injectat prin JS e INVIZIBIL pentru boții AI:** secțiunea `collection-seo-lsi` punea textul LSI într-un `data-seo-content` (atribut escaped) și-l injecta cu `innerHTML` din JS. Google randează JS, dar **GPTBot/PerplexityBot/ClaudeBot NU execută JS** → nu vedeau deloc descrierea (verificat cu `gigi:shopify-geo`: 0 question_headings, 0 citable_passages pe un fetch static). FIX: **server-render** conținutul direct în HTML (`{{ section.settings.seo_content }}` legat de metafield), iar JS face DOAR collapse („Vezi mai mult") + hide-on-filter (progressive enhancement — conținutul rămâne în DOM chiar și ascuns). Verifică cu un fetch `User-Agent: GPTBot` că textul + FAQ + JSON-LD chiar apar în HTML brut. Permite și cei 14 boți AI în robots.txt (`gigi:shopify-geo robots`). NB: scorul GEO e euristic relativ (ratează FAQPage/BreadcrumbList reale) — nu-l urmări orbește, nu-l cita clientului.
...
- **⚠️ Capcane la conținut RO din ȘABLOANE (învățate când clientul a prins erori live — NU publica fără să verifici TOT output-ul):** generarea de descrieri/LSI prin substituția titlului colecției într-un șablon e fragilă în română:
  1. **Acord gen/număr:** NU pune un adjectiv care se acordă imediat după titlu. `„{T} practic și rezistent"` → „Mese și măsuțe practic și rezistent" (greșit). Folosește conectori invariabili (`„{T} de calitate"`, `„{T} pentru …"`, `„{T}: …"`) sau VERBE (`„găsești {T} care rezistă…"`) în loc de adjective.
  2. **Clasificare pe substring:** matching de temă pe substring lovește fals — `„auto"` prinde „aut**o**adeziv" (tapet/parchet → temă „auto" 😅), `„baterii"` prinde „baterii bucătărie" → temă „baie". Verifică keyword-urile SPECIFICE/lungi ÎNAINTEA celor scurte; ordinea contează.
  3. **Capitalizare în text curent:** lowercase doar pe prima literă lasă „Baie & **S**anitare". Lowercase pe fiecare cuvânt DAR păstrează acronimele (LED, 3D) și cifrele.
  4. **Verifică TOT:** generează și tipărește H2/intro/H3 pentru FIECARE pagină (cu titlurile reale) și citește-le, nu valida pe un singur exemplu. Ține FAQ-ul din `faq_jsonld` sincronizat cu FAQ-ul vizibil din LSI (regenerează-le împreună).
...
- **gigi:seo-technical** (management AI-crawlers GPTBot/ClaudeBot) · **gigi:schema** (schema avansat, validare policy/merchant) · **gigi:seo-sxo** (page-type vs intent) · **gigi:seo-cluster** (topic clusters).
