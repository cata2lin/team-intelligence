...
argument-hint: scan <brand> [--days 7] [--only mentions,news,youtube,reddit,gsc,instagram]
---
...
| **youtube** | YouTube Data API v3 (search.list) | video-uri RO care ne pomenesc (canal, titlu, dată) | gata — cere `YOUTUBE_API_KEY` (sau OAuth `youtube.readonly` cu clientul existent) |
| **reddit** | Reddit OAuth (app-only) | fire recente care pomenesc brandul | gata — cere `REDDIT_CLIENT_ID`+`REDDIT_CLIENT_SECRET` (JSON public e 403 chiar și rezidențial) |
| **instagram** | Graph API `ig_hashtag_search` + `/tags` | postări cu #brand + cine ne-a tăguit | gata — cere `IG_GRAPH_TOKEN` cu `instagram_basic` (token actual e ads-only) |
