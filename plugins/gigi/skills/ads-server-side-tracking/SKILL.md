...
> **ARONA (gigi) — de ce contează la noi.** Magazinele noastre COD cu formular Releasit
> (Carpetto/Ofertele/Casa Ofertelor/CZ) **ocolesc checkout-ul nativ → pixelul client-side NU
> trage purchase** (de-aia apar conturi cu „spend dar 0 conversii" în `gigi:gads-audit` → flag
> CODGAP). **Server-side (sGTM + Meta CAPI + Google Enhanced Conversions) recuperează exact aceste
> 30-40% de date** trimițând evenimentul de pe server (din webhook-ul Shopify `orders/create`),
> nu din browser. E nivelul următor după gclid/UTM (deja puse peste tot). Vendorat din MIT
> [claude-ads](https://github.com/AgriciDaniel/claude-ads) (vezi `UPSTREAM_LICENSE`), adaptat:
> leagă-l de `gigi:google-ads-mcc/cod_tracking.py` + webhook-urile Shopify per magazin
> (`gigi:shopify-stores`). Token-uri/CAPI keys → KB, niciodată hardcodate.
