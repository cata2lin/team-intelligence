...
- **`profit_verdict.py`** — SCALE/HOLD/CUT per campanie pe TOATE conturile, **pe PROFIT (breakeven real din `brandref`)**, nu pe CPA-target. Zone: CPA≤`scale_cpa`=SCALE, ≤`breakeven_cpa`=HOLD, >BE=CUT. Corectează mislabeling-ul CPA-target (PMax cu clienți noi la CPA mai mare = tot profitabil). `uv run profit_verdict.py [--days 30]`. Refresh breakeven: `gigi:fulfillment-analytics/breakeven.py --store all` → re-scrie `breakeven_cpa/scale_cpa/breakeven_roas/target_roas` în brandref.
