---
name: google-ads-mcc
description: Read and operate any Google Ads account linked under the team MCC (API v22) — live performance reports, budget/bidding/status/keyword/negative mutations, full Search campaign creation, and the end-to-end video pipeline (upload to YouTube + attach to Performance Max). Plus an optimization playbook and PMax asset-group asset management. Credentials (MCC developer token + OAuth refresh token) come from the `metrics` DB; no per-account login. Read-only by default; mutations are dry-run unless explicitly applied. Use for any live Google Ads work on a brand (Esteban, Belasil, Grandia, …) without screenshots.
---
...
Native Python, se leagă de wrapper-ul v22. Toate READ-ONLY / DRY (doar raportează candidați; aplici cu `add-negatives`/`add-keywords`).
```bash
...
- **API version**: folosim **v22** (v21 blocat aug 2026). `API="v22"` în `gads.py` și toate scripturile.
...
## 4f. Demand Gen (YouTube in-stream = „YouTube ads") — `create-demandgen` + video ad
`gads.py create-demandgen --customer <cid> --name "..." --budget <RON> --geo 2642 --lang 1032 --channels instream` → campanie PAUSED (v25). `--channels instream` = DOAR YouTube in-stream (echivalentul unei campanii Video prin API). Cold-start = **fără `--tcpa`** (Max Conversions; un tCPA pe o campanie NOUĂ o înfometează — adaugi tCPA după ~15-30 conv, sub breakeven). Testat: GT + **Grandia (26-aug)**.
- **geo/lang = `adGroupCriterion`, NU `campaignCriterion`.** Pe Demand Gen, campaignCriterion location/language **pică** (400, trigger `OWNED_AND_OPERATED` / „error code not in this version") — targetarea se pune pe AD GROUP. (Fixat în create-demandgen pasul 2; dacă mai pică: `adGroupCriterionOperation.create {adGroup, location:{geoTargetConstant:"geoTargetConstants/2642"}}` + `{language:{languageConstant:"languageConstants/1032"}}`.)
- **Creative = `DemandGenVideoResponsiveAd`** (`adGroupAdOperation.create`, v25). Câmpuri **OBLIGATORII**: `ad.name` · `businessName:{text}` · `headlines` (≤30) · **`longHeadlines` (≥1, ≤90)** ← lipsa = `collectionSizeError TOO_FEW` · `descriptions` (≤90) · `videos` (**max 5/ad** → 9 videouri = 2 ad-uri) · `logoImages` (≥1, **1:1**) · `finalUrls`. **NU** pune `callToActions`.
- **Video assets:** `assetOperation.create {youtubeVideoAsset:{youtubeVideoId}}` per video (partial=true; dublurile pică benign), apoi rezolvă cu `SELECT asset.resource_name, asset.youtube_video_asset.youtube_video_id FROM asset WHERE asset.type='YOUTUBE_VIDEO'`. Videourile pot fi PUBLICE pe canalul brandului (nu-ți trebuie tokenul canalului) — dar leagă canalul de cont (Linked accounts→YouTube) pt earned views + branding corect.
- **Logo:** `assetOperation.create {type:IMAGE, imageAsset:{data:<base64>}}`; `sips -z 1200 1200` (1:1, ≤5 MB).
- **Ad-urile sunt IMUTABILE** (finalUrls, logo, videos) → ca să schimbi logo/URL: `remove` ad-ul + `create` unul nou (nu update). Șablon: citește structura+copy dintr-un ad Demand Gen care rulează (ex GT camp `24139682773`).
...
- **Non-Brand = VOLUM, nu long-tail.** Un Non-Brand Search „mort" (0 conv, ~0 spend, `rank_lost_IS` mare) e adesea din keyword-uri toate long-tail de 3-4 cuvinte (ex. „parfum lunga durata pret mic") = ~zero volum de căutare → 0 impresii. **NU e bidding/buget — e lipsa de volum.** Fix = `add-keywords` cu **termeni generici cu volum** (parfum dama/barbati, genti dama, apa de parfum) pe PHRASE + Smart Bidding filtrează. (Dovadă iul-2026: Nubra Non-Brand reînviat cu generice; GT Non-Brand avea intent dar nu volum.) ⚠️ Generice = high-volume dar mai puțin țintite → verifică `gigi:search-terms` la ~1 săpt → `add-negatives` (batch — resetează learning).
- **Scalare orizontală când PMax e profitabil DAR necapat.** Un PMax SCALE pe profit (CPA ≤ scale_cpa) care cheltuie **<90% din buget** NU crește pe buget (nu-l folosește — nu e budget-limitat) → crește **orizontal**: campanie Non-Brand Search nouă / mai multe keywords / produse noi în asset group. (Dovadă: Gento PMax profitabil la 73% buget → lansat Gento Non-Brand în loc de +buget.)
...
- 🎯 **Target CPA per magazin (canonic, aprobat user) — Google MAI MIC decât Social.** Folosește target-urile din `brandref` (`target_cpa_google` / `target_cpa_social`; vezi și `shared/HARTA.md` → „Target CPA per magazin"). **REGULĂ: Google rulează la CPA mai mic decât Meta/TikTok** (~65%), fiindcă prinde brand + intenție mare (ieftin) și un tCPA mic disciplinează exact PMax/non-brand-ul scump (Brand Search e oricum sub prag → lasă-l să culeagă tot, nu-l plafona). Caveat: conversiile Google-s umflate (last-click + Shopping App) → tCPA-ul mușcă pe PMax/non-brand, nu pe brand. Setezi cu `set-tcpa` DOAR după 15-30 conv (sub asta = cold-start → doar cap buget). Ex.: parfumuri Google 15 / Social 25; deals Google 18-20 / Social 30. ⚠️ **DAR pt PMax/Shopping/all-channel NU judeca pe CPA-target — judecă pe PROFIT (jos).**
- 💰 **Verdict scale/cut pe PROFIT (canonic — evită mislabeling-ul).** Target-ul CPA agresiv (15/20) etichetează greșit PMax profitabil drept „scump". **`profit_verdict.py`** = SCALE/HOLD/CUT per campanie pe TOATE conturile, cu **breakeven REAL din `brandref`** (`breakeven_cpa`/`scale_cpa`/`breakeven_roas`, populat din `gigi:fulfillment-analytics/breakeven.py`). Zone: **CPA ≤ `scale_cpa`(=BE×0.7, ~30% contribuție)=SCALE** · scale<CPA≤`breakeven_cpa`=HOLD · CPA>BE=CUT. `target_cpa_google/social` rămân DOAR pt **Search non-brand** (eficiență). ⚠️ ROAS-ul Google e umflat ~1.5× → gate-ul robust = **CPA vs breakeven**. Refresh: `breakeven.py --store all` → re-scrie în brandref. Dovadă (iul-2026): Gento/GT PMax = „unprofitable" pe CPA-target 20/15 DAR **SCALE** pe profit (CPA 30/33 << breakeven 61/75); Carpetto = CUT (CPA 56 > BE 51).

### 🚨 CELE 4 CAPCANE care INVERSEAZĂ verdictul de scale/cut — CITEȘTE ÎNAINTE SĂ ATINGI UN BUGET
> Toate patru descoperite pe pielea noastră (13-14 iul 2026). **Fiecare, singură, m-a făcut să recomand EXACT OPUSUL acțiunii corecte** (să tai o campanie profitabilă / să scalez una în saturare). Primele două + a patra sunt reparate în `profit_verdict.py`; **a treia e o schimbare de gândire, niciun script nu te salvează de ea.**

1. **🥇 MEDIE vs MARGINAL — cea mai scumpă și cea mai subtilă.** Un **CPA mediu sănătos poate ascunde o scalare care pierde bani.** Nu întreba „care e CPA-ul?", ci **„cât mă costă comanda ÎN PLUS cumpărată cu bugetul în plus?"**
   → **Grandia (iul-2026):** spend săptămânal **+37%** (23.887→32.798) a adus doar **+12,5% comenzi** (543→611, comenzi REALE din AWBprint) → **cost MARGINAL = 8.911/68 = 131 lei/comandă** vs contribuție (BE) **79** → **scalarea pierdea ~3.500 lei/săptămână**… în timp ce **CPA-ul MEDIU (54 < BE 79) arăta impecabil** și toate uneltele ziceau „SCALE".
   → **REGULĂ: când spend-ul crește mult și conversiile puțin, calculează `Δspend / Δconversii` ÎNAINTE de a mai scala. Dacă marginalul > breakeven → OPREȘTE-TE, chiar dacă media zice că ești profitabil.** (Media e mereu mai frumoasă decât marginalul când ești în saturare.)

2. **MONEDĂ — breakeven e în RON, CPA e în moneda CONTULUI.** `breakeven.py` scoate BE_CPA **în RON** pentru toate magazinele (antetul zice „AOV în RON"), dar Google Ads raportează CPA în **CZK/PLN/EUR** pt conturile non-RO. Comparate direct = **verdict inversat**: Bonhaus CZ ieșea „🔴 CUT, 125 CZK vs BE 37, de 3,4× peste" când **125 CZK = 25 RON < 37 RON = PROFITABIL** (era chiar cel mai sufocat buget din portofoliu, `lostBud` 36%). Reparat: conversie prin `metrics.fx_rates` (BNR) + afișaj `"125 CZK = 25 RON"`. **Nu compara niciodată un prag cu o metrică fără să verifici UNITATEA.**

3. **FEREASTRA de 30z MINTE ÎN AMBELE SENSURI** (nu doar la CUT):
   - *CUT stale* = campania **s-a reparat deja** → Belasil 30z=44 (CUT) dar recent **34** = profitabil; Carpetto 30z=51 dar recent **23 = SCALE** (era sufocată de un buget de 25 lei/zi).
   - *SCALE stale* = campania **se saturează acum** → Grandia 30z=51 zicea „SCALE"… exact când marginalul explodase.
   **NU acționa niciodată pe verdictul ferestrei lungi.** `profit_verdict.py` verifică acum trendul recent la TOATE campaniile.

4. **LAG-UL DE CONVERSIE — ultimele 1-2 zile mint MEREU.** Google atribuie conversiile cu întârziere → CPA-ul zilelor proaspete e fals-umflat. **Dovadă (Grandia 13-iul): Google raporta 58,9 conv → „CPA 85 = peste breakeven = pierdere!" — magazinul avusese REAL 111 comenzi, cea mai bună zi din 2 săptămâni.** De aceea fereastra „recentă" din `profit_verdict.py` = zilele **-8..-3** (sare peste ultimele 2). Cu `LAST_7_DAYS`, lag-ul ar face orice campanie să pară că se saturează → n-ai mai scala nimic niciodată. **Nu judeca pe azi/ieri** (nici intraday: „CPA azi 54 vs ieri 34" = același artefact).

> **+ Variația zilnică e NORMALĂ și mare — nu e semnal.** Grandia, 2 săptămâni, zero schimbări de strategie: **55 → 111 comenzi/zi**. Un ±35% de la o zi la alta e zgomot obișnuit. Când cineva zice „azi merge prost, -60% față de ieri" → **verifică întâi dacă «ieri» n-a fost un vârf** (era: 111 = ziua cea mai bună din fereastră). **O zi singură nu e niciodată un semnal.**
>
> 🔑 **META-REGULA (cea mai importantă): când un verdict pare EXTREM, presupune că unealta MINTE — nu că ai descoperit ceva.** Toate 4 capcanele *inversau* concluzia, nu o nuanțau, iar un verdict autoritar („CUT = pierde bani") suprimă exact îndoiala de care ai nevoie. Ce ne-a salvat de fiecare dată a fost implauzibilitatea: *„un breakeven de 1,5 € pe comandă?"* · *„un **brand search** la 3,4× peste breakeven, când brandul e cea mai ieftină conversie posibilă?"* Verifică **unitatea, fereastra și marginalul** înainte să acționezi.

- **Feed real video** (§4) — PMax with strong video beats asset-only; competitors in most niches run video-heavy.
- **Don't reset learning needlessly** — dar RESETUL NU E TABU dacă merită. Un salt >20% re-intră în learning (1-2 săpt CPA instabil). **Merită resetul DOAR când:** (1) e chiar plafonată de buget, (2) **cererea RAMPĂ** (spend zilnic urcă, nu e plat), (3) headroom ≥2× (CPA ≤ ½ breakeven → rămâne profitabilă și dacă CPA se dublează în relearn), (4) volum elastic (PMax/Shopping, nu Brand). **NU reseta o campanie PLATĂ/demand-limited** (spend zilnic care nu urcă) — resetul e degeaba, bugetul oricum nu s-ar cheltui. Dovadă (15-iul, user OK cu resetul „dacă rezultatele-s bune"): **nubra PMax 320→500** (rampă reală 81→267/7z, CPA 22 vs BE 49) + **gento PMax 280→400** (69→246, CPA 27 vs BE 61) = scalare agresivă corectă; **gt PMax NEatins** (spend plat 160→188 = demand-limited, resetul ar fi fost inutil). 🔑 Semnalul „poate absorbi un salt mare" = **trendul zilnic de spend urcă**, nu doar util%.
- 🔑 **După reset: SEMNALUL e CPA, NU calendarul.** Regula „nu atinge 2 săpt" e prea rigidă (corecție user, 15-iul). Poți **continua să mărești bugetul în pași ≤20% chiar și în câteva zile, DACĂ CPA-ul rămâne bun** — urmezi cererea, ≤20% nu re-resetează. Ce EVIȚI e să **stivui alt salt MARE (>20%) sau să schimbi bidding-ul/target-ul** — alea re-declanșează relearn-ul de fiecare dată și rămâi fără semnal la nesfârșit. Deci: mărire mică + CPA bun = OK oricând; salt mare / bidding = o dată, apoi lasă-l să se așeze.
...
- ⚠️ **„+20%" NU e mărimea pasului — e PLAFONUL care nu resetează learning-ul (20-aug-2026).** Chiar și o campanie CAPATĂ (cheltuie 100%+) poate să NU aibă cerere profitabilă în plus: +20% cumpără licitații scumpe → **CPA face STEP în sus, conv scad**. Probat: Grandia Shopping-New (CPA 40→82, conv 114→61) + Bonhaus CZ PMax — ambele REVERITE. Dimensionează pasul după cererea reală (lostBud%/CPA marginal), nu automat 20%. **REGULĂ: crește MIC sau deloc → urmărește CPA-ul ZILNIC 2-3 zile → STEP în sus = revert imediat.** (`gads-tcpa-magnet-aug2026`)
- 🔴 **PROTOCOL review portofoliu (obligatoriu, 25-aug-2026):** înainte de orice analiză/reparare: **(1)** verifică ZIUA REALĂ cu `date "+%Y-%m-%d %A"` (NU presupune — sesiunea poate dura zile reale); **(2)** `change_history` = ce ai schimbat + CÂND; **(3)** evaluează evoluția CPA/conv ZILNIC DE LA fiecare schimbare (a ajutat / a stricat / irelevant?). GAQL: `LAST_14_DAYS` (NU `LAST_10_DAYS` — invalid). Și pe magazin **COD**: orice CPA-spike **verifică-l vs comenzile REALE (AWBprint `frisbo_created_at`/zi) + `metrics.all_conversions` ÎNAINTE de a tăia** — CPA-ul Google poate fi artefact de tracking. Probat: Grandia „CPA 97" (21-aug) = tracking RUPT (conv 158→44, dar comenzi reale ~105/zi OK) → tăierea bugetului a fost greșită, revert. (`gads-portfolio-review-protocol`, `gads-cod-attribution-gap`)
...
8. **Build** (`build_all.py`): PMax skelet atomic (budget + campanie `maximizeConversions{targetCpaMicros}` + `shoppingSetting.merchantId` + geo RO `2642` + **lang RO `1032`** + asset group + listing group `UNIT_INCLUDED/SHOPPING`) → assets (text + imagini; **BN+LOGO la nivel CAMPANIE** fiindcă Brand Guidelines ON, restul la asset group) → Brand Search → Non-Brand Search → enable PMax.
   ...
   - 🔴 **v22: câmp OBLIGATORIU pe `campaign.create`** → `containsEuPoliticalAdvertising: "DOES_NOT_CONTAIN_EU_POLITICAL_ADVERTISING"` (altfel `REQUIRED` error).
...
- **`profit_verdict.py`** — SCALE/HOLD/CUT per campanie pe TOATE conturile, **pe PROFIT (breakeven real din `brandref`)**, nu pe CPA-target. Zone: CPA≤`scale_cpa`=SCALE, ≤`breakeven_cpa`=HOLD, >BE=CUT. + pt SCALE, **check budget-limited pe AZI** (spend azi vs bugetul CURENT + lost-budget-IS) → **🔥 RAISE** (mai vrea buget) vs **necapat** (crește orizontal). `uv run profit_verdict.py [--days 30]`. 🔑 **SCALE ≠ crește bugetul** — doar dacă e budget-limitat. ⚠️ **LECȚIE (iul-2026, prins de user): verifică budget-limited pe SPEND-ul de AZI vs bugetul NOU, NU media 7z** — după o mărire same-day, media 7z reflectă bugetul VECHI și zice fals „necapat". Bugetul se folosește în 1-3 zile → re-check zilnic, nu stivui same-day; buget ≤20% nu resetează learning → un SCALE încă capat pe bugetul nou merită iar +≤20%. Refresh breakeven: `breakeven.py --store all` → re-scrie în brandref.
- **`pmax_assets.py`** — populează asset group-ul unui PMax creat cu `create-pmax` (care lasă SKELETON fără assets) + îl ENABLE. ⚠️ **GOTCHA Brand Guidelines**: `create-pmax` creează campania cu `brand_guidelines_enabled=TRUE` care e **IMMUTABLE** → business name + logo TREBUIE legate ca **`campaignAssets`** (nivel campanie), nu `assetGroupAssets`, altfel `BRAND_ASSETS_NOT_LINKED_AT_CAMPAIGN_LEVEL`. Restul (headlines/long/desc/marketing+square img) = `assetGroupAssets`. Asset-urile se creează în **2 pași** (create toate → link), NU atomic (validarea dă false `NOT_ENOUGH_*`). Imagini = bytes inline base64. API v22. Ref: bonhaus-pl-google-launch.
