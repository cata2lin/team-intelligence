...
## Portofoliu + populare PMax asset-group prin API (adăugate iul-2026)
- **`gads_portfolio.py`** — performanță pe TOATE conturile MCC dintr-o rulare (spend/conv/CPA/ROAS 30z + trend 7z) vs target CPA per magazin. Winners-ii SUB-SCALAȚI (CPA<target + budget-lost mare → `set-budget`) + conturile peste target. `uv run gads_portfolio.py`.
- **`pmax_assets.py`** — populează asset group-ul unui PMax `create-pmax` (skeleton) + ENABLE. ⚠️ **Brand Guidelines** e `TRUE`+IMMUTABLE → business name+logo ca **`campaignAssets`** (nu assetGroupAssets), altfel `BRAND_ASSETS_NOT_LINKED_AT_CAMPAIGN_LEVEL`. Rest (headlines/desc/img) = `assetGroupAssets`. Assets în **2 pași** (create→link), nu atomic (false `NOT_ENOUGH_*`). API v21. Ref: bonhaus-pl-google-launch.
