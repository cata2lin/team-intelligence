...
# ///
"""Upload local PNG/JPG banners as image assets and link them to a PMax asset group.
Config via env: CIDARG, AGARG (asset_group id), IMGSARG = JSON [["file.png","MARKETING_IMAGE"],...]
relative to DIRARG. Dry-run unless --apply."""
...
r1=post("assets",[{"create":{"name":f"{os.path.basename(f).replace('.png','')}","imageAsset":{"data":b64(os.path.join(DIR,f))}}} for f,_ in IMGS],partial=True)
...
print("2) link la asset group:",r2.status_code,"| linkuri:",len(r2.json().get("results",[])))
pf=r2.json().get("partialFailureError")
if pf: print("   err:",json.dumps(pf["details"][0]["errors"][0],ensure_ascii=False)[:220])
else: print(f"   ✓ {len(names)} imagini atașate la {AG}")
