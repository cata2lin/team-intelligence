...
## ARONA team adaptation (gigi:youtube)

This skill is vendored from the MIT-licensed [claude-youtube](https://github.com/AgriciDaniel/claude-youtube)
(see `UPSTREAM_LICENSE`) and extended for the ARONA team with **write-back automation** on OUR own OAuth.

**`execution/youtube_manage.py`** — list / update / set-visibility / batch-apply on a brand channel,
authenticating with the team's per-brand refresh tokens in the SharedClaude KB
(`YOUTUBE_<BRAND>_REFRESH_TOKEN` + common `YOUTUBE_OAUTH_CLIENT_ID/_SECRET`). Brands with tokens:
BELASIL CARPETTO GENTO GT NUBRA OFERTELE.

```
uv run execution/youtube_manage.py check  --brand OFERTELE          # auth + WRITE-scope test
uv run execution/youtube_manage.py list   --brand GT --json
uv run execution/youtube_manage.py update --brand GT --video <id> --title "..." --desc "..." --privacy public
uv run execution/youtube_manage.py apply  --brand GT --plan plan.json --privacy public
```

**Management scope:** `videos.update` (title/description/tags/visibility) needs `youtube.force-ssl`,
NOT the old upload-only scope. Re-auth a channel with `../google-ads-mcc/yt_oauth.py` setting
`YT_SCOPE="...youtube.upload ...youtube.force-ssl ...youtube.readonly"`, log in to that brand channel,
then `kb.py secret-set YOUTUBE_<BRAND>_REFRESH_TOKEN <token>`. (Consent shows an "unverified app"
warning → Advanced → Continue; the OAuth app is the team's own.) The consulting sub-skills (seo,
metadata, audit, thumbnail, …) generate the copy that youtube_manage.py then writes back.
