...
|---|---|---|---|
| **AWB Arona** (`awb.arona.ro`) | secret `DATABASE_URL_AWBPRINT` | **Sursa de adevăr livrare + transport REAL**: `orders` (status, `total_price`, `line_items`, `currency`, `store_uid`), `order_awbs` (`transport_cost_fara_tva` = cost REAL per AWB, ~69% acoperire, exclus `is_return_label`), `stores` (`uid`,`name`=domeniu public). | `shopify_domain` în `stores` e NULL → nu poți join pe el; mapează prefix→domeniu (vezi profit_core.PREFIX_AWB_DOMAIN); transport „cu TVA"; internațional în valută locală |
...
- **`profit_core.py`** (metrics-cache/scripts + `/root/Scripturi/profit_core.py`) = **SINGLE SOURCE**. Funcții: `vat_for_country/prefix` (RO.21/BG.20/CZ.21/PL.23/HU.27/SK.23/HR.25), `cogs_ron` (override+conversie RON), `parcel_transport` (cascadă: **real AWBprint → media DPD nomenclator → flat**), `refusal_transport_multiplier` (refuz×2), `is_revenue`, `allocate_marketing_by_orders` (CPA uniform), `prefix_brandid`, `PREFIX_AWB_DOMAIN`. **Orice motor de profit nou IMPORTĂ asta.**
...
- **Per brand (P&L canonic)** → `api/profitability.py` (engine, VPS) → `cache.brand_pnl_monthly` → **`gigi:multi-brand-pnl`**. Refresh: `build_cache.py --table brand_pnl_real --apply`.
...
- **Cost transport real per comandă** → AWBprint `order_awbs.transport_cost_fara_tva` (vezi profit_core.parcel_transport).
